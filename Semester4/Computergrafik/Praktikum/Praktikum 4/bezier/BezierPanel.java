...
class BezierPanel extends JPanel {
	...
	public void paintComponent(Graphics graphics) {
		...
		for (int p = 0; p < points.size(); p++) {
			...
			g2d.drawRect((int) (points.get(p).x + 0.5) - 3, (int) (points.get(p).y + 0.5) - 3, 5, 5);

			// Verbinde die Punkte
			if (p > 0) {
				g2d.drawLine((int) (points.get(p - 1).x + 0.5), (int) (points.get(p - 1).y + 0.5),
						(int) (points.get(p).x + 0.5), (int) (points.get(p).y + 0.5));
			}
		}

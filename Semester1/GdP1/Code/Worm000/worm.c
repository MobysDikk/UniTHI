...
int main(void) {
    ...
    char* Fenster = "Das Fenster hat xxx Zeilen und yyy Spalten";
    int msg_len;
    int min_rows = 3;
 ...
 initializeCursesApplication();  // Init various settings of our application
  ...
  msg_len =  strlen(Fenster);       // Compute length of our template
    ...
    if ( LINES < min_rows       || COLS < msg_len     ) {
        ...
        printf("Momentaner Wert: Lines= %d Cols= %d\n", LINES,COLS);
        ...
        nodelay(stdscr, FALSE);                   // make getch to be a blocking call (Hier fehlt noch was)
        getch();
